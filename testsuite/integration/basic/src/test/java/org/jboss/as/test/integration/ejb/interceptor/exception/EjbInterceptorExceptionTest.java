...
public class EjbInterceptorExceptionTest {

    @Deployment
    public static Archive<?> deploy() {

        WebArchive war = ShrinkWrap.create(WebArchive.class, "test.war")
                .addPackage(EjbInterceptorExceptionTest.class.getPackage())
    ...
    @Test
    public void testExceptionNotSuppressedInPreDestroyCallback() {
        Bean<Baz> bean = (Bean<Baz>) beanManager.getBeans(Baz.class).iterator().next();
        CreationalContext<Baz> ctx = beanManager.createCreationalContext(bean);
        Baz baz = bean.create(ctx);
        baz.ping();
        try {
            bean.destroy(baz, ctx);
            fail("Exception in PreDestroy interceptor method was suppressed.");
        } catch (Exception e) {
            // OK
        }
        assertTrue(BazPreDestroyInterceptor.isPreDestroyCalled());
    }
}
